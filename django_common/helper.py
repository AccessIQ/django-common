...
def form_errors_serialize(form):
    ...
    for field in form.fields.keys():
        if field in form.errors:
            if form.prefix:
                errors['{0}-{1}'.format(form.prefix, field)] = force_str(form.errors[field])
            else:
                errors[field] = force_str(form.errors[field])

    if form.non_field_errors():
        errors['non_field_errors'] = force_str(form.non_field_errors())
